 ...
 * 
 * @param conn MySQL connection handle (must be valid)
 * @param users Array to store loaded users (must have space for at least 100 users)
 * @param count Pointer to store number of users loaded (output parameter)
 * @return 1 on success, 0 on failure
 * 
 * @details
 * - Executes SELECT query to get all users
 * - Fetches results row by row
 * - Converts role string to UserRole enum
 * - Maximum 100 users loaded
 * 
 * @note
 * This is optional - currently users are loaded from file (users.txt)
 * You can enhance this to load from MySQL instead if needed
 */
int db_load_users(MYSQL *conn, User *users, int *count) {
    // Step 1: Execute SELECT query to get all users
    ...
    // Step 2: Store query results
    MYSQL_RES *result = mysql_store_result(conn);
    if (!result) {
        *count = 0;
        return 0; // No results or error
    }
    
    // Step 3: Fetch each row and populate users array
    *count = 0;
    MYSQL_ROW row;
 ...
 * 
 * @param conn MySQL connection handle (must be valid)
 * @param user Pointer to User structure containing user data
 * @return 1 on success, 0 on failure
 * 
 * @details
 * - Inserts new user or updates existing user
 * - Uses ON DUPLICATE KEY UPDATE to handle duplicates
 * - Updates password and role if username already exists
 * 
 * @note
 * This is optional - currently users are saved to file (users.txt)
 * You can enhance this to save to MySQL instead if needed
 */
int db_save_user(MYSQL *conn, const User *user) {
    char query[300];
    
    // Build INSERT query with ON DUPLICATE KEY UPDATE
    // If username exists, update password and role
    snprintf(query, sizeof(query),
             "INSERT INTO users (username, password, role) VALUES ('%s', '%s', '%s') "
             "ON DUPLICATE KEY UPDATE password='%s', role='%s'",
             user->username,                    // Username (primary key)
             user->password,                    // Password
             role_to_string(user->role),        // Role ("admin" or "user")
             user->password,                    // Update password if exists
             role_to_string(user->role));       // Update role if exists
